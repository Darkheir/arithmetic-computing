...
class TestListArithmeticCallback(TestCase):
    ...
    def test_call_error(self):
        self._mock_calculator.calculate.side_effect = ArithmeticError("Message", "error_operation")
        result = self._list_callback(['will fail'])
        self.assertEqual(result, ["error(error_operation)"])

    def test_call(self):
        self._mock_calculator.calculate.side_effect = [5, 1.5, ArithmeticError("Message", "error_operation")]
